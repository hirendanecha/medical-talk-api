...
exports.notificationMail = async (userData) => {
  ...
  const mailObj = {
    email: userData.email,
    subject: "MedicalTalk notification",
...
exports.channelNotificationEmail = async (userData) => {
  ...
  let msg = `You have been assign in MedicalTalk channel by the MedicalTalk Admin.
  To access your channel, log into your MedicalTalk account,click on the
  MedicalTalk icon at the top of the page,then click on My Channel.`;
...
exports.communityApproveEmail = async (profileId, isApprove) => {
  ...
  if (userData) {
    ...
    let msg = "";
    if (isApprove === "Y") {
      msg = `MedicalTalk has approved your doctor application.`;
    } else {
      msg = `MedicalTalk has unapproved your doctor application.`;
    }
    let redirectUrl = `${environment.FRONTEND_URL}`;
    const mailObj = {
...
exports.cancelAppointmentNotificationMail = async (id, doctorName) => {
  const query =
    "select u.Email,p.FirstName,p.LastName,p.Username from users as u left join profile as p on p.UserID = u.Id where p.ID =?";
  const values = [id];
  const [data] = await this.executeQuery(query, values);
  let name = data?.Username || userData.FirstName;
  let msg = `Your appointment with ${doctorName} has been cancelled, please book another slot!`;
  let redirectUrl = `${environment.FRONTEND_URL}`;
...
exports.sendAppointmentMailToUser = async (data) => {
  ...
  const query1 =
    "select u.Email,p.FirstName,p.LastName,p.Username from users as u left join profile as p on p.UserID = u.Id where p.ID =?";
  const values1 = [data.doctorProfileId];
...
const getIcalObjectInstance = async (
  ...
  cal.createEvent({
    ...
    url: url, // 'event url',
    summary: "Request video call with Doctors and Surgeons",
    organizer: {
      // 'organizer details'
      name: name,
      email: email,
